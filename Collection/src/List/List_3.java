...
public class List_3 {
    public static void main(String[] args) {
        ...
        // (주의)정수의 리스트인 경우에는 정수를 매개변수에 넣어면 인덱스로 인식함!!
        aList3.remove(1); // 1이 아니라 1번 인덱스의 요소인 5가 삭제 됨
        System.out.println(aList3); // [1, 2, 6]

        // #7.remove(Object o) 해당 요소를 찾아서 삭제
        aList3.remove(Integer.valueOf(2)); // Integer로 타입 변환
        ...
        // #9.isEmpty (요소가 있는지 없는지 확인할 때 사용.) 비었는면 ture 리턴
        ...
        System.out.println(Arrays.toString(objects)); //[1, 2, 3] (출력된 모습은 같지만 배열로 변화 되어 출력됨)

        // #13.어떻게 타입변환을 해야 안전한가?
        Integer[] integers = aList3.toArray(new Integer[0]);
        // new Integer[0]는 0의 length를 가진 배열을 만드는게 아니라
        // aList.size()와 같은 배열크기로 만들라는 약속!
