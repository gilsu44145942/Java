...
select 담당자직위, 도시 , count(*)as 고객수, avg(마일리지)as 평균마일리지 from 고객
group by 담당자직위, 도시
order by 담당자직위, 도시;
...
select 도시,고객번호, sum(마일리지)as 총마일리지 from 고객
where 고객번호 like 'T%'
group by 도시, 고객번호
having 총마일리지 >=1000;

-- 제품별 주문수량합계 구하기
select 제품번호, sum(주문수량)as 주문수량합계 from 주문세부
group by 제품번호 
order by 제품번호;

-- 각 제품별 총주문수량이 1000개 이상인 제품만 출력
select 제품번호, sum(주문수량)as 총주문수량 from 주문세부
group by 제품번호
having 총주문수량>=1000
order by 총주문수량;

-- 제품테이블에서 '아이스크림' 제품들에 대하여 제품명별로 재고합을 출력
select 제품명, sum(재고)as 재고합 from 제품
