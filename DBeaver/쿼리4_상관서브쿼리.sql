...
그러므로 효율적인 설계와 최적화 필요함! 

<MySQL에서 상관서브쿼리의 위치>
select와 where 
from은 불가능함 -MySQL은 from절에서 외부테이블에 대한 참조를 허용하지 않기 때문!
*/ 
...
-- from절의 상관서브쿼리
-- MySQL은 from절에서 외부테이블에 대한 참조는 허용하지 않음
select 고객.고객번호, 고객.고객회사명, 주문요약.최종주문일
from 고객
join 
(select 고객번호, max(주문일)as 최종주문일
from 주문
where 주문.고객번호 = 고객.고객번호 -- 외부 쿼리의 값 참조 (참조 못함)
group by 고객번호)
주문요약 
on 고객.고객번호 = 주문요약.고객번호; -- 코드오류 발생!!
