...
group by year(주문일); -- group by 주문년도;

/* 3. 주문테이블에서 요청일보다 발송이 늦어진 주문내역이 월별로 몇건씩인지 요약하여 출력
 단,	주문월 순서대로 정렬하시오 */
select month (주문일)as 주문월,count(*)as 주문건수 from 주문
where 요청일<발송일
group by month (주문일) -- group by 주문월;
order by month (주문일);
...
select if(마일리지 >=50000, 'vip고객','일반고객')as 고객구분, count(*)as 고객수,avg(마일리지)as 평균마일리지 from 고객
group by 고객구분 -- group by (마일리지 >=50000, 'vip고객','일반고객');
order by 고객구분;
...
select 주문번호, 제품번호 , sum(단가*주문수량)as 주문금액합 from 주문세부
group by 주문번호, 제품번호
order by 주문번호, 제품번호;

-- group_concat 사용예
select 주문번호, group_concat(제품번호 order by 제품번호)as 제품번호목록, sum(단가*주문수량)as 주문금액합 from 주문세부
group by 주문번호
order by 주문번호;
...
select 직위, 이름, count(*)as 사원수 from 사원
group by 직위, 이름
order by 직위, 이름;

-- group_concat 사용예 
select 직위, count(*)as 사원수, group_concat(이름 order by 이름)as 사원이름목록 from 사원
group by 직위
order by 직위;
