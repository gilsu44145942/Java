...
-- 타입변환
-- MySql의 고정 데이터 타입 char, signed, unsigned, date, datetime, time, binary(이진데이터) 
-- 가변 데이터 타입 : varchar, blob(Binary Large Object)
-- cast
-- convert
...
select ifnull(5,'null'), ifnull(null,'값이없음'); 
           
           
-- nullif
-- nullif(A,B)A와 B가 동일하면 null반환, 다르면 A반환
-- 특정 컬럼을 업데이트했을 경우 각 데이터별로 업데이트 여부 확인하는데 사용 
-- 기존 값과 달리 업데이트 되었으면 업데이트된 값을 기록하는 용도
