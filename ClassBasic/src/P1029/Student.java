...
public class Student {
    ...
    // 생성자와 매서드 차이점
    // 1) 목적: 생성자는 필드의 초기화 역할. 매서드는 각종 기능 구현 역할
    // 2) 이름: 생성자는 클래스와 동일한 이름. 매서드는 자유롭게 작명(주로 동사로 시작)
    // 3) 반환형: 생성자는 반환형이 없음. 매서드는 반드시 명시(void, int, String 등)
    // 인텔리제이는 자동으로 생성자를 만들어주는 기능을 지원함
    // 우클릭 - Generate(생성) - Constructor(생성자) - 원하는 필드 선택 - OK
