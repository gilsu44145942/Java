...
public class MysqlJdbcExample {
    ...
    public List<Customer> getAllCustomers() {
        ...
        // 외부에 있는 데이터베이스 연결하는 과정은 반드시 에외처리를 해줘야 함
        // tyr~catch 문법을 사용
        // try()를 실행하고 만약 오류가 발생하면 catch구문을 실행함
        try (
    ...
    // #3. 원하는 정보를 조회하기위한 메서드를 정의 : "모든 부서정보를 조회"
    ...
    // #4. 조인쿼리를 사용하는 메서드 정의
    // 이름, 입사일, 부서명 조회
    // 조인테이블로 결합된 정보가 조회되므로 기존 클래스로에 저장 불가
    public void getEmployees_1() {
        // 1) ResultSet 의 정보를 그대로 사용함
        // 따로 저장하지 않기때문에 재사용성이 좋지 않음
    ...
    public void getEmployees_2() {
        // 2) Map으로 동적 데이터 처리
        // 쿼리결과의 구조가 자바클래스와 일치하지 않고 재사용 예정이라면
    ...
    public void getCustomerNotOrder_1() {
        // 1) ResultSet 의 정보를 그대로 사용함
        // 따로 저장하지 않기때문에 재사용성이 좋지 않음
        ...
        try (
                ...
                ResultSet resultSet = statement.executeQuery(query)) {
            while (resultSet.next()) {
                String customerId = resultSet.getString("고객번호");
                String companyName = resultSet.getString("고객회사명");
    ...
    public void getCustomerNotOrder_2() {
        String query = "select 고객번호, 고객회사명 from 고객 " +
                "where 고객번호 not in (select 고객번호 from 주문)";
        ...
        try (
            ...
            while (resultSet.next()) {
                ...
                customer.put("고객번호", resultSet.getString("고객.고객번호"));
...
//        }
        ...
        System.out.println();
        repository.getCustomerNotOrder_1();
        System.out.println();
        repository.getCustomerNotOrder_2();
    }
}
