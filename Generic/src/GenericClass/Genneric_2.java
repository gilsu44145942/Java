...
//모든클래스의 부모인 Object를 이용하면 문제를 해결할 수 있음
public class Genneric_2 {
    public static void main(String[] args) {
        ...
        // Object는 모든 클래스의 부모클래스로서 모든타입의 객체 저장 가능
        // 자바의 특정상 Object는 약한 타입체크 방식으로 동작됨
        // (다른 참조형 타입들은 엄격한 (=강한) 타입체크 사용)
        // 그러므로 컴파일시점에는 타입이 맞는지 여부를 확인할 수 없으며
        // 런타임시에 실제 타입 기반으로 확인하게 됨
        Pencil pencil2 = (Pencil)goods1.getObject(); // 런타임 에러발생

        // Object를 사용하는 방법은 하나의 클래스를 사용을 가능하게 하고
        // 이전의 문제(Genneric_1.java)는 해결 가능하나
        // 타입변황 오류를 즉시 알 수 없는 단점이 있음!
    }
}
